...
def Ckuser(message):
  ...
  try:
    response = requests.get('https://virusiq.ml/rev/ch.php?id={}'.format(userID)).json()
    if response["ok"]:
      return True
    elif response["ok"] == False:
      kb = InlineKeyboardMarkup([[InlineKeyboardButton("اضغط للاشتراك ⏺", url="t.me/REVTHONIQ")] ])
      Bot("sendMessage",{"chat_id":chatID,"text":"- عذرا عزيزي يجب عليك الاشتراك في","reply_to_message_id":message.id,"parse_mode":"html","disable_web_page_preview":True,"reply_markup":kb})
      return False
